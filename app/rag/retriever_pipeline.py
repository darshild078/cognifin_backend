...
from app.rag.cache_utils import atomic_write_bytes, atomic_write_json, atomic_faiss_write
...
class _ChunkUnpickler(pickle.Unpickler):
    """
    Custom unpickler that remaps Chunk class references from any
    module path to the current module. Handles pickles created on Colab,
    retriever_pipeline, app.rag, etc.
    """
...
class RetrieverPipeline:
    ...
    def __init__(self, model_name: str = None):
        ...
        # Try local cache first for instant boot
        try:
    ...
    def retrieve(self, query: str, top_k: int = None) -> List[RetrievalResult]:
        ...
        Args:
            ...
            top_k: Number of results to return (default: from app.rag.env or 5)
    ...
    def load_index(self, save_dir: str) -> bool:
        ...
        try:
            ...
            if self.index.ntotal != len(self.chunks):
                print(f"⚠️  Index vectors ({self.index.ntotal}) != chunks count ({len(self.chunks)})")
                return False
            
            if self.index.d != self.embedding_dim:
                print(f"⚠️  FAISS dimension ({self.index.d}) != runtime dimension ({self.embedding_dim})")
                return False

            # Load manifest for extra validation if present
            if os.path.exists(manifest_path):
                with open(manifest_path, "r") as f:
                    manifest = json.load(f)
                
                if self.index.ntotal != manifest.get("num_chunks", self.index.ntotal):
                ...
                if stored_sentinels:
                    for doc_id, expected in stored_sentinels.items():
                        current = self.compute_chunk_sentinels(self.chunks, doc_id)
                        current_doc = current.get(doc_id, {})
                        if (current_doc.get("first") != expected.get("first") or
                                current_doc.get("last") != expected.get("last")):
                            print(f"⚠️  Chunk order sentinel mismatch for '{doc_id}' — will re-index")
                            return False
            
            self.is_indexed = True
            logger.info(f"Loaded index from cache: {self.index.ntotal} chunks")
            return True
