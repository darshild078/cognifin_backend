...
from app.rag.cache_utils import atomic_write_bytes, atomic_write_json
from app.rag.retrieval_logger import log_retrieval_event, RetrievalTimer

from app.rag.metadata_schema import (
    ChunkMetadata,
    DocumentRecord,
    create_default_metadata,
)

from app.rag.lookup_index import LookupIndex, ImmutableRangeError, RetrievalScope
from app.rag.search_plan import SearchPlan, MergeStrategy
...
class CorpusManager:
    ...
    def load_registry(self, save_dir: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Registry loaded: {len(self.documents)} document(s), {self._total_vectors} vectors")
            return True
    ...
    def validate_cache_integrity(self, faiss_count: int) -> bool:
        ...
        logger.info(
            f"Cache integrity verified: {faiss_count} vectors across {len(self.documents)} document(s)"
        )
        return True
